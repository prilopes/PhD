...
setwd("C:/Users/Priscilla/Dropbox/PhD/Tese/Experimentos/R/Experiments_BenchGen/Benchmark2_20000/measures")

allFiles <- list.files(pattern = "[.]csv", include.dirs = FALSE)
measures <- foreach(file = allFiles, .combine = 'rbind') %do% sub(".csv", "", file)

selected <- list(
  c(2,9,12,15,30,43,56,69,74,79,80,84), #cRand: higher is better
  c(2,9,12,15,30,43,56,69,74,79,80,84), #F1: higher is better
  NULL, #nMac
  NULL, #nMiC
  c(2,9,12,15,30,43,56,69,74,79,80,84), #precision: higher is better
  c(2,9,12,15,30,43,56,69,74,79,80,84), #purity: 0 = bad, 1 = good
  c(2,9,12,15,30,43,56,69,74,79,80,84), #recall: lower is better
