...
setwd("C:/Users/Priscilla/Dropbox/PhD/Tese/Experimentos/R/Experiments_BenchGen/Benchmark1_11000/measures")

allFiles <- list.files(pattern = "[.]csv", include.dirs = FALSE)
measures <- foreach(file = allFiles, .combine = 'rbind') %do% sub(".csv", "", file)

selected <- list(
  c(2,6,10,13,16,31,44,57,70,75,79,82,86), #cRand: higher is better
  c(2,6,10,13,16,31,44,57,70,75,79,82,86), #F1: higher is better
  NULL, #nMac
  NULL, #nMiC
  c(2,6,10,13,16,31,44,57,70,75,79,82,86), #precision: higher is better
  c(2,6,10,13,16,31,44,57,70,75,79,82,86), #purity: 0 = bad, 1 = good
  c(2,6,10,13,16,31,44,57,70,75,79,82,86), #recall: lower is better
