...
setwd("C:/Users/Priscilla/Dropbox/PhD/Tese/Experimentos/R/Experiments_BenchGen/Benchmark1_5500/measures")

allFiles <- list.files(pattern = "[.]csv", include.dirs = FALSE)
measures <- foreach(file = allFiles, .combine = 'rbind') %do% sub(".csv", "", file)

selected <- list(
  c(2,6,9,12,15,17,32,45,58,71,73,77,81,85), #cRand: higher is better
  c(2,6,9,12,15,17,32,45,58,71,73,77,81,85), #F1: higher is better
  NULL, #nMac
  NULL, #nMiC
  c(2,6,9,12,15,17,32,45,58,71,73,77,81,85), #precision: higher is better
  c(2,6,9,12,15,17,32,45,58,71,73,77,81,85), #purity: 0 = bad, 1 = good
  c(2,6,9,12,15,17,32,45,58,71,73,77,81,85), #recall: lower is better
